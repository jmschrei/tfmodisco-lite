...
@click.option(
    ...
    "--attributions",
    type=click.Path(exists=True),
    help="A .npy or .npz file with the hypothetical attributions for all positions.",
)
@click.option(
    "-i",
    "--h5py",
...
@click.option(
    ...
    default=400,
    show_default=True,
    help="Window around the peak centre considered for motif discovery.",
)
@click.option(
    "-z",
    "--size",
    ...
    default=20,
    show_default=True,
    help="Seqlet core size (sliding window size).",
)
@click.option(
    "-t",
    "--trim-size",
    ...
    default=30,
    show_default=True,
    help="Length to trim patterns to (trim_to_window_size).",
)
@click.option(
    "-f",
    "--seqlet-flank-size",
    ...
    default=5,
    show_default=True,
    help="Flank length added to each extracted seqlet.",
)
@click.option(
    "-g",
    ...
    default=10,
    show_default=True,
    help="Extra flank added when polishing patterns.",
)
@click.option(
    "-j",
    "--final-flank-to-add",
...
@click.option(
    ...
    "--write-tomtom",
    is_flag=True,
    help="Write raw TOMTOM tsvs to the output directory.",
)
@click.option(
    "-s",
    "--suffix",
    default="./",
    show_default=True,
    help="Prefix to prepend to <img> src paths inside the HTML (useful for notebooks).",
)
@click.option(
    "-m",
    "--meme-db",
    type=click.Path(exists=True),
    default=None,
    help="Optional MEME motif database to use when running TOMTOM.",
)
@click.option(
    "-n",
    "--n-matches",
    ...
    type=int,
    show_default=True,
    help="Number of top TOMTOM matches to include per pattern.",
)
@click.option(
    "-l",
    "--lite",
    is_flag=True,
    help="Use tomtom-lite (Euclidean distance) instead of the full TOMTOM tool.",
)
@click.option(
    "-@",
    "--num-cores",
...
@cli.command(help="Output a BED file of seqlets from a modisco results file.")
@click.option(
    "-i",
    "--h5py",
    "h5_path",
...
@click.option(
    ...
    type=click.Path(exists=True),
    required=True,
    help="The path to the peaks file.",
)
@click.option(
    "-c",
    "--chroms",
    ...
    required=True,
    help="A comma-delimited list of chromosomes, or '*' denoting which chromosomes to process.",
)
@click.option("-q", "--quiet", is_flag=True, help="Suppress output to stdout.")
@click.option(
    "-w",
    ...
    type=int,
    default=None,
    help="Optional. This is for backwards compatibility for older modisco h5 files that don't contain the window size as an attribute. This should be set the to size of the window around the peak center that was used for.",
)
def seqlet_bed(
    h5_path: str,
    output: Optional[str],
...
@cli.command(help="Output a FASTA file of seqlets from a modisco results file.")
@click.option(
    "-i",
    "--h5py",
    "h5_path",
...
def seqlet_fasta(
    ...
    """Output a FASTA file of seqlets from a modisco results file."""
